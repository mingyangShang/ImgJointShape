...
from src.data import input_data
...
class AdvCrossModalSimple(BaseModel):
    def __init__(self, model_params):
        ...
        self.y_img = tf.placeholder(tf.int32, [None, self.model_params.n_class])
        self.y_shape = tf.placeholder(tf.int32, [None, self.model_params.n_class])
        # self.y_single = tf.placeholder(tf.int32, [self.model_params.batch_size,1])
        self.l = tf.placeholder(tf.float32, [])
        self.emb_img = self.visual_feature_embed(self.tar_img)
        self.emb_shape = self.label_embed(self.tar_shape)
        ...
        logits_v = self.label_classifier(self.emb_img)
        logits_w = self.label_classifier(self.emb_shape, reuse=True)
        self.label_loss = tf.nn.softmax_cross_entropy_with_logits(labels=self.y_img, logits=logits_v) + \
                          tf.nn.softmax_cross_entropy_with_logits(labels=self.y_shape, logits=logits_w)
        self.label_loss = tf.reduce_mean(self.label_loss)
        self.label_img_pred = tf.argmax(logits_v, 1)
        self.label_img_acc = tf.reduce_mean(tf.cast(tf.equal(self.label_img_pred, tf.argmax(self.y_img, 1)), tf.float32))
        self.label_shape_pred = tf.argmax(logits_w, 1)
        self.label_shape_acc = tf.reduce_mean(tf.cast(tf.equal(self.label_shape_pred, tf.argmax(self.y_shape, 1)), tf.float32))
        self.label_class_acc =  tf.divide(tf.add(self.label_img_acc, self.label_shape_acc), 2.0)
        # TODO no triplet loss
        self.emb_loss = 100*self.label_loss # + self.triplet_loss
        self.emb_v_class = self.domain_classifier(self.emb_img, self.l)
        self.emb_w_class = self.domain_classifier(self.emb_shape, self.l, reuse=True)
    ...
    def train(self, sess):
        self.img_data = input_data.read_data(config.IMG_TRAIN_FEATURE_FILE, config.IMG_TRAIN_LABEL_FILE,
                                             use_onehot=True, label_dim=self.model_params.n_class)
        self.shape_data = input_data.read_data(config.SHAPE_TRAIN_FEATURE_FILE, config.SHAPE_TRAIN_LABEL_FILE,
                                               use_onehot=True, label_dim=self.model_params.n_class)
        # self.check_dirs()
        ...
        for epoch in range(self.model_params.epoch):
            ...
            l = 2. / (1. + np.exp(-10. * p)) - 1
            while batch * self.model_params.batch_size <= max(self.img_data.train.size(), self.shape_data.train.size()):
                # create one-hot labels
                batch_feat, batch_img_labels = self.img_data.train.next_batch(self.model_params.batch_size)
                batch_vec, batch_shape_labels = self.shape_data.train.next_batch(self.model_params.batch_size)
                # TODO no triplet loss
                ...
                _, _, label_loss_val, emb_loss_val, domain_loss_val, domain_class_acc_val, label_class_acc_val = sess.run([emb_train_op, domain_train_op, self.label_loss, self.emb_loss, self.domain_class_loss, self.domain_class_acc, self.label_class_acc],
                          ...
                          self.y_img: batch_img_labels,
                          self.y_shape: batch_shape_labels,
                          # self.y_single: np.transpose([batch_labels]),
                          self.l: l})
                print('Epoch=%d,batch=%d,time: %4.4f, emb_loss: %.8f, domain_loss: %.8f, label_loss: %.8f, domain_class_acc: %.8f, label_class_acc: %.8f' %(
                    epoch+1, batch, time.time() - start_time, emb_loss_val, domain_loss_val, label_loss_val, domain_class_acc_val, label_class_acc_val
                ))
                batch += 1
    ...
    def eval(self, sess):
        self.img_data = input_data.read_data(None, None, config.IMG_EVAL_FEATURE_FILE, config.IMG_EVAL_LABEL_FILE,
                                             use_onehot=True, label_dim=self.model_params.n_class)
        self.shape_data = input_data.read_data(None, None, config.SHAPE_EVAL_FEATURE_FILE, config.SHAPE_EVAL_LABEL_FILE,
                                             use_onehot=True, label_dim=self.model_params.n_class)
        start = time.time()
        self.saver = tf.train.Saver()
        self.load(sess)

        eval_img_feature, eval_img_label = self.img_data.test.next_batch(self.img_data.test.size(), shuffle=False)
        eval_joint_img_feature, eval_img_pred_label, eval_img_label_acc_val = sess.run([self.emb_img, self.label_img_pred, self.label_img_acc],
                                                               feed_dict={self.tar_img:eval_img_feature, self.y_img:eval_img_label})
        ...
        eval_shape_feature, eval_shape_label = self.shape_data.test.next_batch(self.shape_data.test.size(), shuffle=False)
        eval_joint_shape_feature, eval_shape_pred_label, eval_shape_label_acc_val = sess.run([self.emb_shape, self.label_shape_pred, self.label_shape_acc],
                                                                   feed_dict={self.tar_shape:eval_shape_feature, self.y_shape:eval_shape_label})
