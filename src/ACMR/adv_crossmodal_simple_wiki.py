        ...
        for epoch in range(self.model_params.epoch):
            if epoch % 1 ==0:
                ...
                while batch * self.model_params.batch_size <= self.data.train.size():
                    vecs, feats, labels = self.data.train.next_batch(self.model_params.batch_size)
                    # labels_ = labels - np.ones_like(labels)
                    # label_binarizer = sklearn.preprocessing.LabelBinarizer()
                    # label_binarizer.fit(range(max(labels_)+1))
                    # b = label_binarizer.transform(labels_)
                    b = pairwise_input_data.onehot(labels, dim=self.model_params.n_class)
