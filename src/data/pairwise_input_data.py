...
class DataSet(object):
    ...
    def next_batch(self, batch_size, shuffle=True):
        ...
        if start + batch_size > self._num_examples:
            ...
            return np.concatenate((shape_fcs_rest_part, fcs_new_part), axis=0), np.concatenate((img_fcs_rest_part, img_fcs_new_part), axis=0), np.concatenate((labels_rest_part, labels_new_part), axis=0)
        else:
            self._index_in_epoch += batch_size
            end = self._index_in_epoch
            return self._shape_fcs[start:end], self._img_fcs[start:end], self._labels[start:end]


def read_data(train_shape_feature_file=None, train_img_feature_file=None, train_shape_label_file=None, eval_shape_feature_file=None, eval_img_feature_file=None, eval_shape_label_file=None, use_onehot=True, label_dim=10):
    ...
    train_shape_label = np.load(train_shape_label_file) if train_shape_label_file else None
    ...
    eval_shape_label = np.load(eval_shape_label_file) if eval_shape_label_file else None
...
if __name__ == '__main__':
    imgs_data = read_data(config.SHAPE_TRAIN_FEATURE_FILE, config.IMG_TRAIN_FEATURE_FILE, config.SHAPE_TRAIN_LABEL_FILE, label_dim=3)
